...
$(function(){
    ...
    $(".music-list").on("mousemove",".list-item", function() {
        ...
        if(!$(this).data("loadmenu")) {
            ...
            '<div class="list-menu" data-no="' + num + '">' +
                ...
                '<span class="list-icon icon-share" data-function="share" title="点击收藏到我的收藏"></span>' +
            '</div>';
            target.html(html);
    ...
    $(".music-list").on("click",".icon-play,.icon-download,.icon-share", function() {
        ...
        switch($(this).data("function")) {
            ...
            break;
            case "share":   // 收藏
                addToPlayList(musicList[rem.dislist].item[num], 3);
                playerSavedata('myLove', musicList[3].item);  // 保存
                layer.msg("添加到收藏成功")
            break;
        }
        return true;
...
function addHis(music) {
   addToPlayList(music, 2);
    playerSavedata('his', musicList[2].item);  // 保存播放历史列表
}

function addToPlayList(music, playListIndex){
    if(rem.playlist == playListIndex) return true;  // 在播放“播放记录”列表则不作改变

    if(musicList[playListIndex].item.length > 300) musicList[playListIndex].item.length = 299; // 限定播放历史最多是 300 首

    if(music.id !== undefined && music.id !== '') {
        // 检查历史数据中是否有这首歌，如果有则提至前面
        for(var i=0; i<musicList[playListIndex].item.length; i++) {
            if(musicList[playListIndex].item[i].id == music.id && musicList[playListIndex].item[i].source == music.source) {
                musicList[playListIndex].item.splice(i, 1); // 先删除相同的
                i = musicList[playListIndex].item.length;   // 找到了，跳出循环
            }
        }
    }

    // 再放到第一位
    musicList[playListIndex].item.unshift(music);
}
    ...
    for(let i=1; i<musicList.length; i++) {
        ...
        if(i == 1) {    // 正在播放列表
         ...
         // 列表不是用户列表，并且信息为空，需要ajax读取列表
            ...
            // 读取正在播放列表
            var tmp_item = playerReaddata('myLove');
            if(tmp_item) {  // 读取到了正在播放列表
                musicList[3].item = tmp_item;
                mkPlayer.defaultlist = 3;   // 默认显示我的收藏
            }
        }
        // 在前端显示出来
...
function clearDislist() {
    ...
    }else if(rem.dislist == 3) {   // 播放记录
        playerSavedata('myLove', '');  // 清空本地记录
    }
    layer.msg('列表已被清空');
    dataBox("sheet");    // 在主界面显示出音乐专辑
}
